...
opt.batch_size = 20     -- Default 3
opt.train_size = 10000  -- set to 0 or 60000 to use all 60000 training data
opt.test_size = 0      -- 0 means load all data
opt.epochs = 1       -- **approximate** number of passes through the training data 
                       --(see below for the `iterations` variable, which is calculated from this) default 2
...
local testLosses = {}      -- test losses after each iteration/minibatch
...
for i = 1, iterations do
  ...
  local testOutputs = model:forward(test.data)
  local testLoss = criterion:forward(testOutputs, test.labels)
  ...
  if i % 100 == 0 then -- don't print *every* iteration, this is enough to get the gist
      print(string.format("minibatches processed: %6s, trainLoss = %6.6f, testLoss = %6.6f", i, minibatch_loss[1], testLoss))
  end
  ...
  testLosses[#testLosses + 1] = testLoss -- append the new loss
...
-- Should be able to elemntwise compare tensors for equality with torch.eq(a, b)
-- but I have problems with number types

-- TODO: compute test classification error here for the second handin itemlocal numCorrect = 0
...
print(numCorrect)
local classificationError = 1.0 - numCorrect / test.data:size(1)
print(string.format("Test classification error = %6.6f", classificationError))
